...
import cs2103.v15_1j.jolt.antlr4.UserCommandBaseVisitor;
import cs2103.v15_1j.jolt.antlr4.UserCommandParser;
import cs2103.v15_1j.jolt.antlr4.UserCommandParser.OverdueFilterContext;
import cs2103.v15_1j.jolt.command.*;
import cs2103.v15_1j.jolt.searcher.*;

public class JoltCommandVisitor extends UserCommandBaseVisitor<Command> {
	...
	public JoltCommandVisitor(String userCommand) {
	...
	@Override
	public Command visitAddFloatingTask(UserCommandParser.AddFloatingTaskContext ctx) {
		visit(ctx.string());
	...
	public Command visitAddEvent(UserCommandParser.AddEventContext ctx) {
		...
		if (start.isBefore(end)) {
			...
			return new InvalidCommand("Please ensure that the event's" + " ending time is after its starting time");
		}
	}
	...
	@Override
	public Command visitAddEventWithoutDate(UserCommandParser.AddEventWithoutDateContext ctx) {
		visit(ctx.string());
		dateTime = LocalDate.now().atStartOfDay();
		visit(ctx.time(0));
		LocalDateTime start = dateTime;
	...
	public Command visitDelCmd(UserCommandParser.DelCmdContext ctx) {
		...
		return new DeleteCommand(itemNum.charAt(0), Integer.parseInt(itemNum.substring(1)));
	}

	@Override
	public Command visitMarkDoneCmd(UserCommandParser.MarkDoneCmdContext ctx) {
		String itemNum = ctx.ITEM_NUM().getText().toLowerCase();
		return new MarkDoneCommand(itemNum.charAt(0), Integer.parseInt(itemNum.substring(1)));
	}

	@Override
	public Command visitUnmarkCmd(UserCommandParser.UnmarkCmdContext ctx) {
		String itemNum = ctx.ITEM_NUM().getText().toLowerCase();
		return new UnmarkCommand(itemNum.charAt(0), Integer.parseInt(itemNum.substring(1)));
	}
	...
	public Command visitAliasAdd(UserCommandParser.AliasAddContext ctx) {
		if (ctx.aliasable(1) == null) {
			...
			return new AliasAddCommand(ctx.WORD().getText().toLowerCase(), ctx.aliasable(0).getStart().getType(),
					ctx.aliasable(0).getText());
		} else {
			return new InvalidCommand(
					"\"" + ctx.aliasable(1).getText() + "\"" + " is already a keyword and cannot be made an alias!");
		}
	}
	...
	@Override
	public Command visitShowHideCmd(UserCommandParser.ShowHideCmdContext ctx) {
		if (ctx.OVERDUE() != null) {
	...
	@Override
	public Command visitSaveLocationCmd(UserCommandParser.SaveLocationCmdContext ctx) {
		visit(ctx.string());
	...
	@Override
	public Command visitFullDateWordMonth(UserCommandParser.FullDateWordMonthContext ctx) {
		int day = Integer.parseInt(ctx.INT(0).getText());
	...
	@Override
	public Command visitFullDateWordMonthMonthFirst(UserCommandParser.FullDateWordMonthMonthFirstContext ctx) {
		int day = Integer.parseInt(ctx.INT(0).getText());
		int month = getMonth(ctx.MONTH_NAME());
		int year;
		if (ctx.INT(2) != null) {
	...
	@Override
	public Command visitTimeWithoutPeriod(UserCommandParser.TimeWithoutPeriodContext ctx) {
		int hour = Integer.parseInt(ctx.INT(0).getText());
		int minute = ctx.INT(1) == null ? 0 : Integer.parseInt(ctx.INT(1).getText());
		dateTime = dateTime.with(LocalTime.of(hour, minute));
		return null;
	}

	@Override
	public Command visitTimeWithPeriod(UserCommandParser.TimeWithPeriodContext ctx) {
		int hour = Integer.parseInt(ctx.INT(0).getText());
	...
	private int process12Hour(int hour, boolean isPm) {
		if ((hour > 12) || (hour <= 0)) {
			throw new DateTimeException("Invalid time, please use correct 12-hour format: " + hour);
		}
	...
	@Override
	public Command visitTimeRangeFilter(UserCommandParser.TimeRangeFilterContext ctx) {
		visit(ctx.time());
		if (ctx.BEFORE() == null) {
			// after command
	...
	public Command visitTimeFilter(UserCommandParser.TimeFilterContext ctx) {
		...
		filters.add(new TimeFilter(time.minusMinutes(30), time.plusMinutes(30)));
		return null;
	}

	@Override
	public Command visitBetweenTimeFilter(UserCommandParser.BetweenTimeFilterContext ctx) {
		visit(ctx.time(0));
		LocalTime start = dateTime.toLocalTime();
		visit(ctx.time(1));
		LocalTime end = dateTime.toLocalTime();
	...
	@Override
	public Command visitDateRangeFilter(UserCommandParser.DateRangeFilterContext ctx) {
		visit(ctx.date());
		if (ctx.BEFORE() == null) {
			// after command
			filters.add(new DateTimeFilter(dateTime.with(LocalTime.MIN), LocalDateTime.MAX));
		} else {
			// before command
			filters.add(new DateTimeFilter(LocalDateTime.MIN, dateTime.with(LocalTime.MAX)));
		}
		return null;
	}
	...
	public Command visitDateFilter(UserCommandParser.DateFilterContext ctx) {
		visit(ctx.date());
		filters.add(new DateTimeFilter(dateTime.with(LocalTime.MIN), dateTime.with(LocalTime.MAX)));
		return null;
	}

	@Override
	public Command visitBetweenDateFilter(UserCommandParser.BetweenDateFilterContext ctx) {
		visit(ctx.date(0));
	...
	@Override
	public Command visitDateTimeRangeFilter(UserCommandParser.DateTimeRangeFilterContext ctx) {
		visit(ctx.datetime());
		if (ctx.BEFORE() == null) {
			// after command
	...
	@Override
	public Command visitBetweenDateTimeFilter(UserCommandParser.BetweenDateTimeFilterContext ctx) {
		visit(ctx.datetime(0));
		LocalDateTime start = dateTime;
		visit(ctx.datetime(1));
		LocalDateTime end = dateTime;
	...
	@Override
	public Command visitThisWeekFilter(UserCommandParser.ThisWeekFilterContext ctx) {
		LocalDateTime now = LocalDateTime.now();
		filters.add(new DateTimeFilter(now.with(DayOfWeek.MONDAY).with(LocalTime.MIN),
				now.with(DayOfWeek.SUNDAY).with(LocalTime.MAX)));
		return null;
	}

	@Override
	public Command visitNextWeekFilter(UserCommandParser.NextWeekFilterContext ctx) {
		LocalDateTime nextWeek = LocalDateTime.now().plusWeeks(1);
		filters.add(new DateTimeFilter(nextWeek.with(DayOfWeek.MONDAY).with(LocalTime.MIN),
				nextWeek.with(DayOfWeek.SUNDAY).with(LocalTime.MAX)));
		return null;
	}

	@Override
	public Command visitThisMonthFilter(UserCommandParser.ThisMonthFilterContext ctx) {
		LocalDateTime now = LocalDateTime.now();
		filters.add(new DateTimeFilter(now.withDayOfMonth(1).with(LocalTime.MIN),
				now.withDayOfMonth(1).plusMonths(1).minusDays(1).with(LocalTime.MAX)));
		return null;
	}

	@Override
	public Command visitNextMonthFilter(UserCommandParser.NextMonthFilterContext ctx) {
		LocalDateTime nextMonth = LocalDateTime.now().plusMonths(1);
		filters.add(new DateTimeFilter(nextMonth.withDayOfMonth(1).with(LocalTime.MIN),
				nextMonth.withDayOfMonth(1).plusMonths(1).minusDays(1).with(LocalTime.MAX)));
		return null;
	}
